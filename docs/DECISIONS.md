...
## 2026-09-20 — Alternate examples must change consequential reasoning

**User direction:** Adithya explicitly checked that worked examples were different from the tasks students must complete independently. This was already the intended requirement; the check exposed gaps in how it had been verified.

**Finding:** All sixteen examples had separate names and data, but that did not establish a distinct problem. The circuit, archive, council and plan-revision examples retained overly similar decision patterns. The permission matrix and parts of A2 also needed stronger contrasts. Lab 1 repeated the assigned hurried-departure inference. The existing catalog test checked a difference label, not educational separation.

**Decision:** Preserve transferable methods and submission standards, while changing evidence or constraints that affect the answer. One beacon now has two simultaneous faults; an unsigned request cannot replace current authority; equipment maintenance does not imply research-reading permission; adequate capacity and a corroborated claim support a different council agreement; a release-authority change requires reordering checks while all routes stay open. A2 now diagnoses a powered-but-dark load and admits two valid mirrors before a larger content scope changes the deliverable. Lab 1 distinguishes time of day from unsupported elapsed time.

**Implementation rule:** Compare every example with assigned browser and campus cases, including phase variants. Carry revisions through controls, frames, narrated reasoning, finished artifacts and Fieldwork. Keep exact assigned-answer comparisons in the internal review rather than inserting an answer sequence into a student-facing difference label. Tests protect concrete contrasts, while content judgement remains a separate review.

**Evidence:** [The complete comparison](EXAMPLE-SEPARATION-AUDIT.md) records all sixteen examples, verification and limits. It includes the honest finding that earlier labels were insufficient. The student's PROCESS account is preserved; no student playtest or publication is inferred. This entry belongs to the unsigned local implementation commit containing it.
