---
title: "Hacking the Impossible Machine"
description: "Distinguish evidence from permission in an explicit model. Explain why invalid actions leave the current state unchanged."
week: 4
learningOutcomes: ["LO2","LO3"]
activityId: "interpreter"
caseFiles: ["E04"]
teachers: ["mara-voss"]
date: 2027-03-19
checkpointDue: 2027-03-19T17:00:00+11:00
produces: ["interpreter-model"]
requires: ["seal-model"]
sampleAvailable: true
assessmentIds: ["fieldwork","assignment-1"]
related: ["lectures/week-04","assessments/assignment-1","assessments/fieldwork"]
spec:
  - "produce interpreter-model with a source-backed explanation"
  - "state one assumption, alternative or unresolved question"
  - "save or export the record; browser progress is not a submission"
---

## Before you arrive

Inspect E04 and revise your A1 ledger (20 minutes). Bring one uncertainty you cannot settle from the supplied files.

Bring seal model. If you are entering this week directly, use the labelled example in the activity. It supplies a starting point and does not count as your original work.

## The two-hour lab

| Time | Work |
| --- | --- |
| 0–15 min | Read the case file and name the decision the evidence affects. |
| 15–35 min | Trace a valid run and an invalid run, writing the state after every attempted action. Use the result to finish the A1 dependency model. |
| 35–70 min | Complete the playable exercise below; record a reason before inspecting the feedback. |
| 70–95 min | Exchange explanations. Challenge one assumption using a specific source. |
| 95–110 min | Revise the output and explain which part of the feedback changed it. |
| 110–120 min | Save or export the record and write the closing reflection. |

## What leaves the room

**interpreter-model** is this week's named artefact. This completes A1’s system analysis and gives the pair project a method for diagnosing changes.

Include your decision, the case-file identifier, a justification, and one remaining limitation. The fields in the exercise are a starting structure; expand them in your assessment document where the brief requires it.

## How to judge the response

‘It saw the record, so it should allow the transfer’ imports an unstated permission rule. A plan is only defensible against the rules actually supplied. State any extra premise instead of hiding it in natural language.

A strong response explains a particular choice and an alternative. The supplied exemplar is inspectable feedback, not a sentence you must copy. Your confidence or reflection is never assigned an automatic academic grade.

## Closing reflection

Where did your explanation previously confuse knowledge with permission?

## Afterwards

Export a backup from the [Operation Room](/operation/). This lab record contributes to [Weekly Fieldwork](/assessments/fieldwork/); the final mark uses your best ten records. The [individual dossier brief](/assessments/assignment-1/) explains its larger use. See [policies](/policies/) for alternatives and missed work.
