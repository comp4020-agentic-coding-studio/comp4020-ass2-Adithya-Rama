...
export function registerImmersiveScene(root: HTMLElement) {
  ...
  function releaseCursor() {
    ...
    message(finePointer() ? 'Controls active. Select Mouse look to look around. Scroll the scene to zoom; Esc exits.' : 'Drag the scene to look. Use Zoom + / Zoom −. Complete the mission in this panel.');
  ...
  function fallbackLook() {
    ...
    message('Move the mouse over the scene to look; use the cursor in the mission panel. Scroll zooms. Esc exits.');
  }
  function startLook() {
    const target = canvas();
    if (!expanded || !target || !finePointer()) { showControls(); return; }
    root.dataset.worldMouseLook = 'true';
    lookButton.setAttribute('aria-pressed', 'true');
    target.focus({preventScroll: true});
    message('Move mouse to look · Scroll to zoom · Tab for mission controls · Esc exits fullscreen.');
    try {
      if (target.requestPointerLock) {
        const request = target.requestPointerLock() as Promise<void> | undefined;
        request?.catch(fallbackLook);
      } else fallbackLook();
    } catch { fallbackLook(); }
  }
  ...
  function open() {
    ...
    // Pointer lock first: fullscreen consumes transient user activation.
    // https://www.w3.org/TR/pointerlock-2/
    if (finePointer()) startLook(); else showControls();
  ...
  lookButton.addEventListener('click', startLook);
  ...
  document.addEventListener('pointerlockerror', () => { if (expanded) fallbackLook(); });
  ...
  // Clicking the scene resumes look; controls keep their normal pointer behaviour.
  root.addEventListener('click', (event) => {
    if (expanded && event.target === canvas() && root.dataset.worldMouseLook !== 'true') startLook();
  });
